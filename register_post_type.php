...
if( !class_exists('TS_Portfolios') ){
	class TS_Portfolios{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(600, 0);
			$this->add_image_size();
		...
		function add_image_size(){
			...
			if( !isset($_wp_additional_image_sizes[$this->thumb_size_name]) ){
				add_image_size($this->thumb_size_name, $this->thumb_size_array[0], $this->thumb_size_array[1], false);
			}
		}
		...
		function register_custom_fields (){
			if( isset($_POST['ts_porfolio_permalink']) )
			{
				update_option('ts_porfolio_permalink', sanitize_title_with_dashes($_POST['ts_porfolio_permalink']) );
			}

			add_settings_section('ts_portfolio_section', 'Portfolio permalinks', array( $this, 'portfolio_section_callback'), 'permalink'); 
			add_settings_field('ts_porfolio_permalink', esc_html('Custom base') , array($this, 'permalink_field_callback'), 'permalink', 'ts_portfolio_section');
		}

		function permalink_field_callback() {
			$option = get_option('ts_porfolio_permalink');

			echo '<input type="text" value="' . esc_attr( $option ) . '" name="ts_porfolio_permalink" id="ts_porfolio_permalink" class="regular-text" />';
		}

		function portfolio_section_callback() {
			echo 'If you want to change the permalink of the portfolio, please fill in the field below';
		}
	
		function change_post_type_slug($args, $post_type){
			$option = get_option('ts_porfolio_permalink');

			if ( $this->post_type === $post_type && $option != '' ) {
...
if( !class_exists('TS_Team_Members') ){
	class TS_Team_Members{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(440, 440);
			$this->add_image_size();
...
if( !class_exists('TS_Testimonials') ){
	class TS_Testimonials{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(120, 120);
			$this->add_image_size();
	...
	class TS_Logos{
		...
		function setting_page_content(){
			$options_default = array(
							...
							,'responsive' => array(
								'break_point'	=> array(0, 480, 991, 1279, 1420)
								,'item'			=> array(2, 3, 5, 6, 7)
							)
						);
...
if( !class_exists('TS_Product_Brands') ){
	class TS_Product_Brands{
		function __construct(){
			...
			/* Register field permalink */
			add_action('load-options-permalink.php', array($this, 'register_custom_fields'));
			add_filter('register_taxonomy_args', array($this, 'change_taxonomy_slug'), 10, 2);
		}
		...
		function register_custom_fields (){
			if( isset($_POST['ts_brands_permalink']) ){
				update_option('ts_brands_permalink', sanitize_title_with_dashes($_POST['ts_brands_permalink']) );
			}

			add_settings_field('ts_brands_permalink', esc_html('Brands base') , array($this, 'permalink_field_callback'), 'permalink', 'optional');
		}

		function permalink_field_callback() {
			$option = get_option('ts_brands_permalink');



			echo '<input type="text" value="' . esc_attr( $option ) . '" name="ts_brands_permalink" id="ts_brands_permalink" class="regular-text" />';
		}

		function change_taxonomy_slug( $args, $taxonomy ){
			$option = get_option('ts_brands_permalink');

			if( 'ts_product_brand' === $taxonomy && $option != '' ){
				$args['rewrite']['slug'] = $option;
			}
